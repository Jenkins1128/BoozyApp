...
export default class RestaurantPage extends React.Component{
    ...
    onFocusFunction = () => {
        ...
        axios.post('https://qvsn1ge17c.execute-api.us-east-2.amazonaws.com/latest/api/menu/list', {
    ...
    onStarRatingPress(rating) {
        ...
        const axios = require('axios').default;
        axios.post('https://qvsn1ge17c.execute-api.us-east-2.amazonaws.com/latest/api/user/'+this.state.restaurantId+'/restaurants', {
            content: rating,
    ...
    getDataFromMenuItem(){
        ...
        axios.post('https://qvsn1ge17c.execute-api.us-east-2.amazonaws.com/latest/api/user/'+this.state.restaurantId+'/menu', {
    ...
    favorite(){
        const axios = require('axios').default;
        axios.post(`https://qvsn1ge17c.execute-api.us-east-2.amazonaws.com/latest/api/user/add`, {
            restaurantId: this.state.restaurantId,
            name: this.state.restaurantName
          }, {
            headers: {
    ...
    render(){
        ...
        return (  
            ...
            <Image source={{ uri: this.state.restaurantImage }} style={styles.logoImage} >
            </Image>
            <Text style={styles.headerText}> {this.state.restaurantName} </Text>
            <Text style={styles.categoriesText}>{this.state.allCategories} • {this.state.rating}★</Text>
            <Text style={styles.phoneNumberText}>{this.state.phoneNumber} • {this.state.reviews} Yelp Reviews</Text>
            ...
            <Animated.View style={[styles.subView,{transform: [{translateY: this.state.bounceValue}]}]} >
                ...
                <TextInput style={styles.priceStyle}  keyboardType={'numeric'} multiline={false} value='' returnKeyType="next"    
                onChangeText={(price) => this.setState({price})  }
                    value={this.state.price.toString()} placeholder="Enter Price" />
                <TextInput style={styles.descriptionStyle} multiline={false} returnKeyType="next"    
                onChangeText={(description) => this.setState({description})  }
                    value={this.state.description} placeholder="Enter menu item" />
            </Animated.View>
